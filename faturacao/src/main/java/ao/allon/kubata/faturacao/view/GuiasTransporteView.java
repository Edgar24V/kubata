...
public class GuiasTransporteView extends VBox {
    ...
    public GuiasTransporteView(FaturaService faturaService, ClienteService clienteService, ProdutoService produtoService, JasperReportService jasperReportService, ModalService modalService) {
    ...
    private HBox createHeader() {
        ...
        Label title = new Label("Guias de Transporte");
        title.getStyleClass().addAll(Styles.TITLE_2);
        title.setFont(Font.font("System", 24));
        Label subtitle = new Label("Emissão, consulta, cancelamento e exportação de guias de transporte");
    ...
    private VBox createFiltersAndTable() {
        ...
        Button cancelar = new Button("Cancelar", IconUtils.icon(Feather.TRASH_2, IconUtils.SIZE_SMALL));
        cancelar.getStyleClass().addAll(Styles.DANGER);
        cancelar.setOnAction(e -> handleCancelar());

        toolbar.getChildren().addAll(imprimir, exportPdf, exportXls, new Separator(Orientation.VERTICAL), cancelar);
    ...
    private void refreshAsync() {
        ...
        Thread t = new Thread(() -> {
            try {
                ...
                List<Fatura> guias = faturaService.buscarGuiasTransporte(inicio, fim, clienteId, status);
                Platform.runLater(() -> {
                    baseData = guias;
                ...
                Platform.runLater(() -> AlertUtils.showExceptionAlert("Erro", "Falha ao carregar Guias de Transporte.", ex));
            } finally {
                Platform.runLater(() -> showLoading(false));
            }
        }, "guias-transporte-reload");
        t.setDaemon(true);
        t.start();
    }
    ...
    private ContextMenu buildRowMenu(Fatura f) {
        ...
        MenuItem miCancelar = new MenuItem("Cancelar", IconUtils.icon(Feather.TRASH_2, IconUtils.SIZE_SMALL));
        miCancelar.setOnAction(e -> handleCancelar());
        return new ContextMenu(miImprimir, new SeparatorMenuItem(), miPdf, miXls, new SeparatorMenuItem(), miCancelar);
    }
    ...
    private VBox createEmptyState() {
        ...
        Label sub = new Label("Nenhuma guia de transporte encontrada com os filtros atuais.");
    ...
    private void handleNova() {
        NovaGuiaTransporteView view = new NovaGuiaTransporteView(faturaService, clienteService, produtoService, modalService);
        view.setPrefWidth(760);
        modalService.create()
    ...
    private void handleImprimir() {
        ...
        try {
            ...
            jp.setName("GT_" + (comp.getNumero() != null ? comp.getNumero().replace("/", "_") : ""));
    ...
    private void handleExportarPdfLista() {
        try {
            ...
            JasperPrint jp = jasperReportService.prepararListaGuiasRemessa(lista); // Reusing template for now
            jp.setName("Lista_Guias_Transporte");
            FileChooser fc = new FileChooser();
            fc.setTitle("Salvar Lista de Guias (PDF)");
            fc.getExtensionFilters().add(new FileChooser.ExtensionFilter("PDF Files", "*.pdf"));
            fc.setInitialFileName("guias_transporte.pdf");
    ...
    private void handleExportarExcelLista() {
        try {
            ...
            fc.setInitialFileName("guias_transporte.xls");
            File file = fc.showSaveDialog(getScene().getWindow());
            if (file != null) {
    ...
    private void handleCancelar() {
        ...
        boolean confirm = AlertUtils.showConfirmationAlert("Cancelar Guia",
                "Confirma cancelar a Guia " + (f.getNumero() != null ? f.getNumero() : "") + "?");
        if (!confirm) return;
        try {
            String motivo = "Cancelada pela gestão de Guias de Transporte";
            faturaService.cancelarFatura(f.getId(), motivo);
            AlertUtils.showInfoAlert("Sucesso", "Guia cancelada com sucesso.");
            refreshAsync();
        } catch (Exception ex) {
            AlertUtils.showExceptionAlert("Erro", "Falha ao cancelar guia.", ex);
        }
    }
}
